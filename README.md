...
Доступ выдаётся по новому **OC5-TRY-токену** после оплаты. Каждый токен действует 7 дней и перед установкой обязательно проверяется онлайн:

1. Открой **[@AITeamVIPBot](https://t.me/AITeamVIPBot)** в Telegram → /start
2. Оплати по ссылке (Prodamus) — бот пришлёт OC5-TRY-токен (привязан к твоему Telegram)
3. Запусти команду установки с этим токеном

Токен подписан Ed25519 (тот же механизм, что у платных тарифов), а его
актуальность проверяется через защищённый сервис статуса. В токене зашит твой
TG ID, поэтому отдать его другу нельзя (бот ответит только тебе).
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/tonytrue92-beep/openclaw-test-drive/main/scripts/install-trial.sh) --token OC5-TRY-XXXX --vps
```

Можно и без `--token` — установщик спросит токен интерактивно. Или
через переменную: `TRIAL_TOKEN=OC5-TRY-XXXX bash <(curl …)`.
...
1. **Доступ** — проверяет OC5-TRY-токен онлайн до начала установки, затем подпись Ed25519 после
...
4. **Telegram** — подключает бота (токен из @BotFather). TG ID владельца
   берётся **из OC5-TRY-токена** — клиента не спрашивают (бот уже знает его).
...
- **Этот репо (`openclaw-test-drive`)** — тест-драйв (1 агент,
  лидогенерация). По OC5-TRY-токену из @AITeamVIPBot — выдаётся после оплаты (Prodamus), действует 7 дней.
